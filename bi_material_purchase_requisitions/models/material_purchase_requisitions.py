...
class RequisitionLine(models.Model):
    ...
    location_id = fields.Many2one('stock.location', string='Location', auto_join=True, ondelete='restrict', readonly=True, domain=[('usage', 'like', 'internal')],default=lambda self: self.env['stock.quant'].search([('product_id', '=', self.product_id.id)], limit=1)))
